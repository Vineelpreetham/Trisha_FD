...
interface CircularGalleryProps
  ...
  /**
   * Direction of the scroll when using the mouse wheel.
   * @default "left"
   */
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          // Hide pixels outside the image bounds (for 'contain' effect)
          ...
          // Multiply by color.a to respect transparent PNGs
...
class App {
  ...
  onWheel(e: WheelEvent) {
    ...
    const directionMultiplier = this.scrollDirection === "right" ? -1 : 1;
    this.scroll.target += (delta > 0 ? this.scrollSpeed : -this.scrollSpeed) * directionMultiplier * 0.2;
  ...
  onMouseMove(e: MouseEvent) {
    ...
    // Map mouse X from screen pixels to viewport units
    const relX = e.clientX - rect.left;
    this.mouseX = ((relX / rect.width) - 0.5) * this.viewport.width;
  }
  ...
  update() {
    ...
    // Detect hovered plane and apply pop scale
    let newHovered = -1;
    if (this.medias && this.mouseX !== -9999) {
      let closestDist = Infinity;
      this.medias.forEach((media, i) => {
        const dist = Math.abs(media.plane.position.x - this.mouseX);
        const halfW = media.plane.scale.x / 2;
        if (dist < halfW && dist < closestDist) {
          closestDist = dist;
          newHovered = i;
        }
      });
    ...
    this.container.style.cursor = newHovered !== -1 ? "pointer" : "grab";

    if (this.medias) {
      this.medias.forEach((media, i) => {
        media.update(this.scroll, direction);
        // Smooth pop scale on hover
        const targetScale = i === this.hoveredIndex ? 1.14 : 1.0;
        media.plane.scale.x = lerp(media.plane.scale.x, media.plane.scale.x * (targetScale / (media as any)._lastTargetScale || 1), 0.1);
        const baseScaleX = (this.viewport.width * (900 * this.screen.height / 800)) / this.screen.width;
        const baseScaleY = (this.viewport.height * (900 * this.screen.height / 800)) / this.screen.height;
        (media as any)._targetScaleX = lerp((media as any)._targetScaleX ?? baseScaleX, baseScaleX * targetScale, 0.12);
        (media as any)._targetScaleY = lerp((media as any)._targetScaleY ?? baseScaleY, baseScaleY * targetScale, 0.12);
        media.plane.scale.x = (media as any)._targetScaleX;
        media.plane.scale.y = (media as any)._targetScaleY;
      });
    }
  ...
  addEventListeners() {
    ...
    this.container.addEventListener("mouseleave", () => { this.mouseX = -9999; this.hoveredIndex = -1; });
  }
